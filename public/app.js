...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const fecharPdf = document.getElementById('fecharPdfPreview');
  const imprimirPdf = document.getElementById('imprimirPdfPreview');
  ...
  document.getElementById('botaoNotificacoes').addEventListener('click', alternarPainelNotificacoes);
  document.getElementById('fecharNotificacoes').addEventListener('click', alternarPainelNotificacoes);
  document.getElementById('ativarNotificacoesNavegador').addEventListener('click', ativarNotificacoesNavegador);
  fecharPdf.addEventListener('click', fecharVisualizacaoPdf);
  imprimirPdf.addEventListener('click', imprimirVisualizacaoPdf);
...
function preencherTelaLote(dados) {
  ...
  const dataPeso = dados.dataPesoEstimado || dataHojePortal();
  definirTexto('dataPesoEstimado', `Peso correspondente a ${dataPeso}`);
  definirTexto('dataPesoAtualProjecao', `Referência: ${dataPeso}`);
  definirTexto('pesoAtualProjecao', adicionarUnidade(formatarNumeroExibicao(dados.pesoEstimado), 'kg'));
  ...
  preencherHistoricoMortes(dados);
  preencherNotificacoes(dados);
...
function atualizarResultadoCarcacaFinal() {
  ...
  const pesoAtual = converterNumero(dadosLoteAtual?.pesoEstimado);
  const pesoFinal = converterNumero(dadosLoteAtual?.pesoFinalProjetado || pesoFinalNumerico);

  atualizarCardRendimento('Atual', pesoAtual, rendimento);
  atualizarCardRendimento('Final', pesoFinal, rendimento);
}

function atualizarCardRendimento(tipo, pesoVivo, rendimento) {
  const sufixo = tipo === 'Atual' ? 'Atual' : 'Final';
  if (!pesoVivo || pesoVivo <= 0) {
    definirTexto(`carcaca${sufixo}Kg`, '—');
    definirTexto(`carcaca${sufixo}Arrobas`, '—');
    return;
  }
  const pesoCarcaca = pesoVivo * (rendimento / 100);
  const arrobas = pesoCarcaca / 15;
  definirTexto(`carcaca${sufixo}Kg`, `${formatarNumero(pesoCarcaca, 2)} kg`);
  definirTexto(`carcaca${sufixo}Arrobas`, `${formatarNumero(arrobas, 2)} @`);
}
    ...
    elemento.innerHTML = `
      ...
      <div class="tratamento-dados">
        <div class="tratamento-dado">
          ...
          <strong>
            ${escaparHtml(primeiroValor(item, ['quantidadeAnimais','QuantidadeAnimais','QuantidadedeAnimais','quantidade'], '0'))}
          </strong>
        </div>
        ...
        <div class="tratamento-dado">
          ...
          <strong>
            ${escaparHtml(
              adicionarUnidade(primeiroValor(item, ['dosePorAnimal','DosePorAnimal','dosePorAnimalML','DosePorAnimalML','dose'], ''), 'ml')
            )}
          </strong>
        </div>
        ...
        <div class="tratamento-dado">
          ...
          <strong>
            ${escaparHtml(
              adicionarUnidade(primeiroValor(item, ['volumeTotal','VolumeTotal','volumeTotalML','VolumeTotalML','volume'], ''), 'ml')
            )}
          </strong>
        </div>
...
function primeiroValor(objeto, campos, padrao = '') {
...
function dataHojePortal() {
  return new Date().toLocaleDateString('pt-BR');
}

function preencherHistoricoMortes(dados) {
  const container = document.getElementById('historicoMortes');
  const mortes = Array.isArray(dados.mortalidadeHistorico)
    ? dados.mortalidadeHistorico
    : (Array.isArray(dados.movimentacoes) ? dados.movimentacoes.filter(item => String(item.tipo || item.Tipo || '').toUpperCase() === 'MORTE') : []);
  container.innerHTML = '';
  if (!mortes.length) {
    container.innerHTML = '<p class="estado-vazio">Nenhuma morte registrada para este lote.</p>';
    return;
  }
  mortes.slice().reverse().forEach(function(item) {
    const card = document.createElement('article');
    card.className = 'morte-item';
    card.innerHTML = `
      <div class="morte-topo"><div><strong>${escaparHtml(primeiroValor(item,['data','Data'],'Data não informada'))}</strong><span>Registro de mortalidade</span></div><strong class="morte-quantidade">${escaparHtml(primeiroValor(item,['quantidade','Quantidade'],'0'))} cabeça(s)</strong></div>
      <div class="morte-dados"><div><span>Motivo</span><strong>${escaparHtml(primeiroValor(item,['motivo','Motivo'],'Não informado'))}</strong></div><div><span>Observações</span><strong>${escaparHtml(primeiroValor(item,['observacoes','Observacoes','Observações'],'—'))}</strong></div></div>`;
    container.appendChild(card);
  });
}

function montarEventosPortal(dados) {
  const eventos = [];
  const tratamentos = dados.sanidade?.tratamentos || [];
  tratamentos.forEach(item => eventos.push({
    id: `trat-${primeiroValor(item,['idTratamento','IDTratamento'], `${item.data}-${item.produto}`)}`,
    tipo: 'tratamento', icone: '💉', titulo: 'Tratamento realizado',
    texto: `${primeiroValor(item,['produto','Produto'],'Produto')} em ${primeiroValor(item,['quantidadeAnimais','QuantidadeAnimais','QuantidadedeAnimais'],'0')} animal(is)`,
    data: primeiroValor(item,['data','Data'],'')
  }));
  const mortes = dados.mortalidadeHistorico || [];
  mortes.forEach(item => eventos.push({
    id: `morte-${primeiroValor(item,['idMovimentacao','IDMovimentacao'], `${item.data}-${item.quantidade}`)}`,
    tipo: 'morte', icone: '⚠️', titulo: 'Morte registrada',
    texto: `${primeiroValor(item,['quantidade','Quantidade'],'0')} cabeça(s) — ${primeiroValor(item,['motivo','Motivo'],'motivo não informado')}`,
    data: primeiroValor(item,['data','Data'],'')
  }));
  return eventos.reverse();
}

function preencherNotificacoes(dados) {
  const eventos = montarEventosPortal(dados);
  const lista = document.getElementById('listaNotificacoes');
  const chave = `notificacoes_lidas_${dados.id || dados.lote || 'lote'}`;
  const lidas = new Set(JSON.parse(localStorage.getItem(chave) || '[]'));
  lista.innerHTML = eventos.length ? '' : '<p class="estado-vazio">Nenhuma notificação para este lote.</p>';
  eventos.forEach(evento => {
    const item = document.createElement('article');
    item.className = `notificacao-item ${lidas.has(evento.id) ? 'lida' : 'nova'}`;
    item.innerHTML = `<span class="notificacao-icone">${evento.icone}</span><div><strong>${escaparHtml(evento.titulo)}</strong><p>${escaparHtml(evento.texto)}</p><small>${escaparHtml(evento.data || '')}</small></div>`;
    lista.appendChild(item);
  });
  const novas = eventos.filter(e => !lidas.has(e.id));
  const badge = document.getElementById('badgeNotificacoes');
  badge.textContent = String(novas.length);
  badge.classList.toggle('oculto', novas.length === 0);
  if (novas.length && 'Notification' in window && Notification.permission === 'granted') {
    const e = novas[0]; new Notification(e.titulo, { body: e.texto, icon: '/icon-192.png' });
  }
}
...
function alternarPainelNotificacoes() {
  ...
  painel.setAttribute('aria-hidden', String(!abrindo));
  if (abrindo && dadosLoteAtual) {
    const eventos = montarEventosPortal(dadosLoteAtual);
    const chave = `notificacoes_lidas_${dadosLoteAtual.id || dadosLoteAtual.lote || 'lote'}`;
    localStorage.setItem(chave, JSON.stringify(eventos.map(e => e.id)));
    document.getElementById('badgeNotificacoes').classList.add('oculto');
    document.querySelectorAll('.notificacao-item').forEach(el => el.classList.remove('nova'));
  }
}
...
async function ativarNotificacoesNavegador() {
  ...
  alert(permissao === 'granted' ? 'Avisos ativados neste aparelho.' : 'A permissão não foi concedida.');
}
...
function exportarLotePDF() {
  ...
  frame.srcdoc = montarHtmlPDF(relatorio);
  preview.classList.remove('oculto');
  preview.setAttribute('aria-hidden', 'false');
  document.body.style.overflow = 'hidden';
}

function fecharVisualizacaoPdf() {
  const preview = document.getElementById('pdfPreview');
  const frame = document.getElementById('pdfPreviewFrame');
...
function imprimirVisualizacaoPdf() {
  const frame = document.getElementById('pdfPreviewFrame');
