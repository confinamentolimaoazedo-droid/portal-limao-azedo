...
function instalarEstilosV652() {
  ...
  style.textContent = `
    ...
    .mercado-fallback-v671 {
      display: grid;
      gap: 10px;
      padding: 14px;
    }

    .mercado-fallback-item-v671 {
      display: flex;
      align-items: center;
      justify-content: space-between;
      gap: 12px;
    ...
    .mercado-fallback-item-v671 strong {
      display: block;
      font-size: 14px;
    }

    .mercado-fallback-item-v671 span {
      color: var(--muted, #68756d);
      font-size: 12px;
    }
...
function instalarMercadoBoiGordoV67() {
  ...
  card.innerHTML = `
    ...
    <p class="mercado-descricao-v67">
      Contrato contínuo e próximos vencimentos do
      futuro de boi gordo negociado na B3.
    </p>
    ...
    <p class="mercado-aviso-v67">
      ...
      As cotações podem apresentar atraso conforme
      as regras da bolsa e do provedor.
  ...
  carregarWidgetMercadoV671();
}

function carregarWidgetMercadoV671() {
  ...
  const tickers = document.createElement('tv-tickers');
  ...
  tickers.setAttribute('direction', 'vertical');
  tickers.setAttribute('item-size', 'compact');
  ...
  carregarScriptTradingViewV671()
    .catch(function() {
      renderizarFallbackMercadoV671(
        destino,
        simbolos
      );
    });
...
function carregarScriptTradingViewV671() {
  const existente = document.querySelector(
    'script[data-tradingview-tickers-v671]'
  );
  ...
  return new Promise(function(resolve, reject) {
    ...
    script.dataset.tradingviewTickersV671 = 'sim';
...
function renderizarFallbackMercadoV671(
  destino,
  simbolos
) {
  if (!destino) {
  ...
  const nomes = {
    'BMFBOVESPA:BGI1!': 'Contrato contínuo',
    'BMFBOVESPA:BGIN2026': 'Jul/2026',
    'BMFBOVESPA:BGIQ2026': 'Ago/2026',
    'BMFBOVESPA:BGIU2026': 'Set/2026',
    'BMFBOVESPA:BGIV2026': 'Out/2026',
    'BMFBOVESPA:BGIX2026': 'Nov/2026',
    'BMFBOVESPA:BGIZ2026': 'Dez/2026'
  };

  destino.innerHTML = `
    <div class="mercado-fallback-v671">
      ${simbolos.map(function(simbolo) {
        const codigo = simbolo.split(':')[1];

        return `
          <a
            class="mercado-fallback-item-v671"
            href="https://www.tradingview.com/symbols/${simbolo.replace(':', '-')}/"
            target="_blank"
            rel="noopener noreferrer"
          >
            <div>
              <strong>${escaparHtmlV652(codigo)}</strong>
              <span>${escaparHtmlV652(nomes[simbolo] || '')}</span>
            </div>
